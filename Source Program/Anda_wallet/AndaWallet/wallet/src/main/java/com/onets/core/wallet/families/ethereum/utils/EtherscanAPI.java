...
import com.onets.core.wallet.families.ethereum.interfaces.LastIconLoaded;
...
public class EtherscanAPI {
    ...
    /**
     * Retrieve all internal transactions from address like contract calls, for normal transactions @see rehanced.com.simpleetherwallet.network.AndaChainAPI#getNormalTransactions() )
     *
    ...
    /**
     * Retrieve all normal ether transactions from address (excluding contract calls etc, @see rehanced.com.simpleetherwallet.network.AndaChainAPI#getInternalTransactions() )
     *
     * @param address Ether address
     * @param b       Network callback to @see rehanced.com.simpleetherwallet.fragments.FragmentTransactions#update() or @see rehanced.com.simpleetherwallet.fragments.FragmentTransactionsAll#update()
     * @param force   Whether to force (true) a network call or use cache (false). Only true if user uses swiperefreshlayout
    ...
    public void getBalance(String address, Callback b) throws IOException {
        Log.e("------------------", "getBalance:  token :  "+token );
        get("http://api.etherscan.io/api?module=account&action=balance&address=" + address + "&apikey=" + token, b);
    }

    public void getBlockNumber(Callback b) throws IOException{
        get("https://api.etherscan.io/api?module=proxy&action=eth_blockNumber&apikey=" + token, b);
    }
    ...
    public void getTransactionByHash(String txHash, Callback b) throws IOException {
        get("https://api.etherscan.io/api?module=proxy&action=eth_getTransactionByHash&txhash="+txHash+"&apikey=" + token, b);
    }
