 ...
 * 安达钱包独立key
 */
public class AndaWalletSingleKey extends BitWalletBase {
    private static final Logger log = LoggerFactory.getLogger(WalletPocketHD.class);

    @VisibleForTesting
    protected SimpleKeyChain keys;

    public AndaWalletSingleKey(CoinType coinType, ECKey key) {
    ...
    public AndaAddress getAddress() {
        lock.lock();
        try {
            return AndaAddress.from(type, keys.getKey(null));
        } finally {
            lock.unlock();
        }
    }
