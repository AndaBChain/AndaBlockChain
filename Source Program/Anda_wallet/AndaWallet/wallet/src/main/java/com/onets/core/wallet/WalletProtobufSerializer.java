...
import com.onets.core.wallet.families.bitcoin.BitTransaction;
...
import com.onets.core.wallet.families.ethereum.EthereumFamilyWalletProtobufSerializer;
import com.onets.core.wallet.families.ripple.RippleFamilyWallet;
import com.onets.core.wallet.families.ripple.RippleFamilyWalletProtobufSerializer;
...
public class WalletProtobufSerializer {
    private static final String TAG = "WalletProtobufSerialize";
    ...
    public static Protos.Wallet toProtobuf(Wallet wallet) {
        ...
        for (WalletAccount account : wallet.getAllAccounts()) {
            ...
            } else if (account instanceof EthereumFamilyWallet) {
                pocketProto = EthereumFamilyWalletProtobufSerializer.toProtobuf((EthereumFamilyWallet) account);
            } else if (account instanceof WalletPocketHD_Anda) {
                pocketProto = WalletPocketProtobufSerializer_Anda.toProtobuf((WalletPocketHD_Anda) account);
            } else if (account instanceof RippleFamilyWallet) {
                pocketProto = RippleFamilyWalletProtobufSerializer.toProtobuf((RippleFamilyWallet) account);
            } else {
    ...
    public static Wallet readWallet(Protos.Wallet walletProto) throws UnreadableWalletException {
        ...
        if (walletProto.hasSeed()) {
            ...
            if (key.hasSecretBytes()) {
                ...
                Log.i(TAG, "readWallet:  key.hasSecretBytes() mnemonic size:"+mnemonic.size());

                seed = new DeterministicSeed(new byte[16], mnemonic, 0);

            } else if (key.hasEncryptedData()) {
                Log.i(TAG, "readWallet:  key.hasEncryptedData() ");
                ...
                if(seed == null){
                    Log.i(TAG, "readWallet   EncryptedSeed : null");
                }
            } else {
        ...
        EthereumFamilyWalletProtobufSerializer ethereumPocketSerializer = new EthereumFamilyWalletProtobufSerializer();
        WalletPocketProtobufSerializer_Anda anDaChainFamilyWalletProtobufSerializer = new WalletPocketProtobufSerializer_Anda();
        RippleFamilyWalletProtobufSerializer rippleFamilyWalletProtobufSerializer = new RippleFamilyWalletProtobufSerializer();
        ...
        for (Protos.WalletPocket pocketProto : walletProto.getPocketsList()) {
            ...
            if (type instanceof BitFamily) {
                ...
                pocket = ethereumPocketSerializer.readWallet(pocketProto,crypter);
            } else if (type instanceof AndaFamily) {
                pocket = anDaChainFamilyWalletProtobufSerializer.readWallet(pocketProto,crypter);
            } else if (type instanceof RippleFamily) {
                pocket = rippleFamilyWalletProtobufSerializer.readWallet(pocketProto,crypter);
            } else {
                throw new UnreadableWalletException("Unsupported type " + type);
            }
            Log.d(TAG, "readWallet: --------------addAccount2018951635");
