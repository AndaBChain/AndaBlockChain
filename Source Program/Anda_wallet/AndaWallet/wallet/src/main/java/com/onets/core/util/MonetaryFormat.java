...
import com.onets.core.coins.FiatValue;
...
public final class MonetaryFormat implements Serializable {
    ...
    private static final String DECIMALS_PADDING = "000000000000000000"; // a few more than necessary for Bitcoin （16个0）修改成了18
    ...
    public CharSequence format(Monetary monetary, int smallestUnitExponent) {
        ...
        Log.e("============= 1 ", "format: maxDecimals   " + maxDecimals + "  smallestUnitExponent   " + smallestUnitExponent);
        checkState(maxDecimals <= smallestUnitExponent);

        // rounding
        long satoshis = Math.abs(monetary.getValue());
        Log.e("=============  2 ", "format: preparation satoshis   " + satoshis
                + "\nsmallestUnitExponent - shift - maxDecimals  :" + (smallestUnitExponent - shift - maxDecimals));
        ...
        Log.e("=============   3 ", "format: satoshis   " + satoshis
                + " \n precisionDivisor   " + precisionDivisor);
        ...
        Log.e("=============   4 ", "format: smallestUnitExponent - shift :  " + (smallestUnitExponent - shift)
                + " \n shiftDivisor   " + shiftDivisor
                + " \n numbers   " + numbers
                + " \n decimals   " + decimals
        ...
        Log.e("=============   5 ", "format: decimalsStr   " + decimalsStr);
        //下面的步骤  00000000  ----->  0.00
        ...
        Log.e("=============  ", "format: result :"+str );

        return str;
    }

    public CharSequence formatRipple(Monetary monetary, int smallestUnitExponent,boolean isSend) {
        // preparation
        int maxDecimals = minDecimals;
        if (decimalGroups != null) {
            for (int group : decimalGroups) {
    ...
    public CharSequence formatEthSign(MyMonetary monetary, int smallestUnitExponent) {
        // preparation
        int maxDecimals = minDecimals;
        if (decimalGroups != null) {
            for (int group : decimalGroups) {
        ...
        // rounding
        BigDecimal satoshis = new BigDecimal(String.valueOf(monetary.getBigValue().abs()));
        Log.e("----------  2 ", "format: preparation satoshis   " + satoshis
                + "\nsmallestUnitExponent - shift - maxDecimals  :" + (smallestUnitExponent - shift - maxDecimals));

        BigDecimal precisionDivisor = BigDecimal.valueOf(10).pow(smallestUnitExponent - shift - maxDecimals);
        BigDecimal x = satoshis.divide(precisionDivisor, roundingMode);
        satoshis = x.multiply(precisionDivisor);
        Log.e("----------  3 ", "format: satoshis   " + satoshis
                + " \n precisionDivisor   " + precisionDivisor);

        // shifting
        BigDecimal shiftDivisor = BigDecimal.valueOf(10).pow(smallestUnitExponent - shift);
        BigDecimal numbers = satoshis.divide(shiftDivisor);
        BigDecimal decimals = satoshis.remainder(shiftDivisor) ;
        Log.e("----------  4 ", "format: smallestUnitExponent - shift :  " + (smallestUnitExponent - shift)
                + " \n shiftDivisor   " + shiftDivisor
                + " \n numbers   " + numbers
                + " \n decimals   " + decimals
        ...
        BigInteger conver = decimals.toBigInteger();
        String decimalsStr = String.format(Locale.US, "%0" + (smallestUnitExponent - shift) + "d", conver);
        Log.e("----------   5 ", "format: decimalsStr   " + decimalsStr);
        //下面的步骤  00000000  ----->  0.00
        StringBuilder str = new StringBuilder(decimalsStr);
        while (str.length() > minDecimals && str.charAt(str.length() - 1) == '0')
        ...
        if (str.length() > 0)
            str.insert(0, decimalMark);
        str.insert(0, numbers);
        if (monetary.getBigValue().compareTo(BigInteger.ZERO) == -1) {
            str.insert(0, negativeSign);
        }else if (positiveSign != 0) {
            str.insert(0, positiveSign);
        }else if(negativeSign > 0){
            str.insert(0, negativeSign);
        }
    ...
    public CharSequence formatEth(MyMonetary monetary, int smallestUnitExponent) {
        // preparation
        int maxDecimals = minDecimals;
        if (decimalGroups != null) {
            for (int group : decimalGroups) {
...
//    }
    ...
    public Value parseEth(ValueType type, String str) throws NumberFormatException {
        return Value.valueOfEth(type, parseValueEth(str, type.getUnitExponent()));
    }

    /**
     * Parse a human readable fiat value to a {@link com.onets.core.coins.Value} instance.
     *
     * @throws NumberFormatException if the string cannot be parsed for some reason
     */
    ...
    private String parseValueEth(String str, int smallestUnitExponent) {
        checkState(DECIMALS_PADDING.length() >= smallestUnitExponent);
        if (str.isEmpty())
            throw new NumberFormatException("empty string");
        char first = str.charAt(0);
