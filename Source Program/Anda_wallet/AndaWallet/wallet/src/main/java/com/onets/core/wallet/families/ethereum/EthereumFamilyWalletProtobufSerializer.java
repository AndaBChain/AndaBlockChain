 ...
 * @author Yu K.Q.
 */
final public class EthereumFamilyWalletProtobufSerializer {
    private static final Logger log = LoggerFactory.getLogger(EthereumFamilyWalletProtobufSerializer.class);

    // Used for de-serialization
    protected Map<ByteString, Transaction> txMap = new HashMap<ByteString, Transaction>();

    public static Protos.WalletPocket toProtobuf(EthereumFamilyWallet account) {
    ...
    public EthereumFamilyWallet readWallet(Protos.WalletPocket walletProto, @Nullable KeyCrypter keyCrypter) throws UnreadableWalletException {
        CoinType coinType;
        try {
        ...
        EthereumFamilyWallet pocket;
        if (walletProto.hasId()) {
            pocket = new EthereumFamilyWallet(walletProto.getId(), rootKey, coinType);
        } else {
            pocket = new EthereumFamilyWallet(rootKey, coinType);
        }
        ...
        Log.e("-------EthereumFamily..", "readWallet: "+pocket.getBalance());
        return pocket;
    }
}
