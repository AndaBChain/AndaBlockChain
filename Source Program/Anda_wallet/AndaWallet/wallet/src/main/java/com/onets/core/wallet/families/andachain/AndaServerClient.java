...
import com.onets.stratumj.StratumClient;
import com.onets.stratumj.messages.CallMessage;
import com.onets.stratumj.messages.ResultMessage;
 ...
 * 安达服务客户端实现类
 */
public class AndaServerClient implements AndaBlockChainConnection {
    private static final Logger log = LoggerFactory.getLogger(com.onets.core.network.ServerClient.class);
...
//        connectionExec.setRemoveOnCancelPolicy(true);
     ...
     * 安达服务客户端
     * @param coinAddress
     * @param connectivityHelper
     */
    public AndaServerClient(CoinAddress coinAddress, ConnectivityHelper connectivityHelper) {
...
//    }
    ...
    private void broadcastOnConnection() {
        for (final ListenerRegistration<ConnectionEventListener> registration : eventListeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
                    registration.listener.onConnection(AndaServerClient.this);
                }
            });
        }
    }
     ...
     */
    @Override
    public void subscribeToAddresses(List<AbstractAddress> addresses, final TransactionEventListener<AndaTransaction> listener) {
        checkNotNull(stratumClient);
        ...
        StratumClient.SubscribeResultHandler addressHandler = new StratumClient.SubscribeResultHandler() {
            ...
            public void handle(CallMessage message) {
                try {
                    AbstractAddress address = AndaAddress.from(type, message.getParams().getString(0));
     ...
     */
    @Override
    public void getUnspentTx(AddressStatus status, TransactionWatcherWallet_Anda listener) {

    }

    @Override
    public void getUnspentTx(final AddressStatus status,
                             final AndaTransactionEventListener listener) {
        checkNotNull(stratumClient);
        ...
        Futures.addCallback(result, new FutureCallback<ResultMessage>() {
            ...
            public void onSuccess(ResultMessage result) {
                ...
                ImmutableList.Builder<com.onets.core.network.ServerClient.UnspentTx> utxes = ImmutableList.builder();
                try {
                    for (int i = 0; i < resTxs.length(); i++) {
                        utxes.add(new com.onets.core.network.ServerClient.UnspentTx(resTxs.getJSONObject(i)));
                    }
                } catch (JSONException e) {
                    onFailure(e);
                    return;
    ...
    public void getHistoryTx(final AddressStatus status,
                             final TransactionEventListener<AndaTransaction> listener) {
        checkNotNull(stratumClient);
        ...
        Futures.addCallback(result, new FutureCallback<ResultMessage>() {
            ...
            public void onSuccess(ResultMessage result) {
                JSONArray resTxs = result.getResult();
                ImmutableList.Builder<com.onets.core.network.ServerClient.HistoryTx> historyTxs = ImmutableList.builder();
                try {
                    for (int i = 0; i < resTxs.length(); i++) {
                        historyTxs.add(new com.onets.core.network.ServerClient.HistoryTx(resTxs.getJSONObject(i)));
                    }
                } catch (JSONException e) {
                    onFailure(e);
                    return;
                ...
                public void run() {
                    ...
                    if (txCachedFile.exists()) {
                        try {
                            ...
                            AndaTransaction tx = new AndaTransaction(type, txBytes);
     ...
     * @param listener
     */
    private void getTransactionFromNetwork(final Sha256Hash txHash, final TransactionEventListener<AndaTransaction> listener) {
        checkNotNull(stratumClient);
     ...
     */
    @Override
    public void getBlock(final int height, final TransactionEventListener<AndaTransaction> listener) {
        checkNotNull(stratumClient);
     ...
     */
    @Override
    public void broadcastTx(final AndaTransaction tx,
                            @Nullable final TransactionEventListener<AndaTransaction> listener) {
        checkNotNull(stratumClient);
     ...
     */
    @Override
    public boolean broadcastTxSync(final AndaTransaction tx) {
        checkNotNull(stratumClient);
    ...
    public static class HistoryTx {
        ...
        public static List<com.onets.core.network.ServerClient.HistoryTx> historyFromArray(JSONArray jsonArray) throws JSONException {
            ImmutableList.Builder<com.onets.core.network.ServerClient.HistoryTx> list = ImmutableList.builder();
            for (int i = 0; i < jsonArray.length(); i++) {
                list.add(new com.onets.core.network.ServerClient.HistoryTx(jsonArray.getJSONObject(i)));
            }
            return list.build();
        }
    ...
    public static class UnspentTx extends com.onets.core.network.ServerClient.HistoryTx {
        ...
        public static List<com.onets.core.network.ServerClient.HistoryTx> unspentFromArray(JSONArray jsonArray) throws JSONException {
            ImmutableList.Builder<com.onets.core.network.ServerClient.HistoryTx> list = ImmutableList.builder();
            for (int i = 0; i < jsonArray.length(); i++) {
                list.add(new com.onets.core.network.ServerClient.UnspentTx(jsonArray.getJSONObject(i)));
            }
            return list.build();
        }
        ...
        public boolean equals(Object o) {
            ...
            AndaServerClient.UnspentTx unspentTx = (AndaServerClient.UnspentTx) o;
