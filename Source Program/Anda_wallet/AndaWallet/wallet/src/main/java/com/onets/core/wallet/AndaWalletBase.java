...
import static com.onets.core.util.AndaAddressUtils.isP2SHAddress;

/**
 * @author Yu K.Q.
 * 安达钱包基础类
 */
abstract public class AndaWalletBase extends TransactionWatcherWallet_Anda implements KeyBag {
    private static final Logger log = LoggerFactory.getLogger(AndaWalletBase.class);

    private final TransactionCreator_Anda transactionCreator;

    AndaWalletBase(CoinType coinType, String id) {
        super(checkNotNull(coinType), id);
        transactionCreator = new TransactionCreator_Anda(this);
    }
     ...
     */
    public AndaSendRequest sendCoinsOffline(AndaAddress address, Value amount)
            throws WalletAccountException {
        return sendCoinsOffline(address, amount, (KeyParameter) null);
    }

    /**
     * {@link #sendCoinsOffline(AndaAddress, Value)}
     */
    public AndaSendRequest sendCoinsOffline(AndaAddress address, Value amount,
    ...
    public AndaSendRequest sendCoinsOffline(AndaAddress address, Value amount,
        ...
        AndaSendRequest request = AndaSendRequest.to(address, amount);
    ...
    public void signMessage(SignedMessage unsignedMessage, @Nullable KeyParameter aesKey) {
        ...
        try {
            ECKey key;
            try {
                AndaAddress address = AndaAddress.from(type, unsignedMessage.getAddress());
    ...
    public void verifyMessage(SignedMessage signedMessage) {
        try {
            ...
            byte[] expectedPubKeyHash = AndaAddress.from(type, signedMessage.address).getHash160();
    ...
    public SendRequest getEmptyWalletRequest(AbstractAddress destination)
        ...
        return AndaSendRequest.emptyWallet((AndaAddress) destination);
    }
    ...
    public SendRequest getSendToRequest(AbstractAddress destination, Value amount)
            throws WalletAccountException {
        checkAddress(destination);
        return AndaSendRequest.to((AndaAddress) destination, amount);
    }

    private void checkAddress(AbstractAddress destination) throws WalletAccountException {
        if (!(destination instanceof AndaAddress)) {
            throw new WalletAccountException("Incompatible address" +
                    destination.getClass().getName() + ", expected " + AndaAddress.class.getName());
        }
    }
     ...
     *
     * @param req a AndaSendRequest that contains the incomplete transaction and details for how to make it valid.
     * @throws WalletAccountException if the request could not be completed due to not enough balance.
     * @throws IllegalArgumentException if you try and complete the same SendRequest twice
     */
    public void completeTransaction(AndaSendRequest req) throws WalletAccountException {
        lock.lock();
        try {
     ...
     */
    public void signTransaction(AndaSendRequest req) {
        lock.lock();
        try {
