...
public class ResponseParser {
    ...
    public static String parseBlockNumber(String response, int comma) throws JSONException {
        Log.e("------------  json ", "parseBlockNumber: "+response);
        if(response.substring(0,1).equals("{")) {
            JSONObject result = new JSONObject(response);
            String blockNumber = result.getString("result").substring(2);
            return new BigInteger(blockNumber, 16).toString();
        }else{
            return "0";
        }
    }
