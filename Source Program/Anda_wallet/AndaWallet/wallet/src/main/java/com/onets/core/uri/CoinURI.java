...
public class CoinURI implements Serializable {
    ...
    public CoinURI(@Nullable CoinType uriType, String input) throws CoinURIParseException {
        ...
        if (!addressToken.isEmpty()) {
            ...
            for (CoinType possibleType : possibleTypes) {
                if (possibleType instanceof EthereumFamily) {
                    try {
                        address = new EthereumAddress(possibleType, addressToken);
                        putWithValidation(FIELD_ADDRESS, address);
                        break;
                    } catch (RuntimeException e) {  /* continue */ }
                } else if (possibleType instanceof AndaFamily) {
                    try {
                        address = new AndaAddress(possibleType, addressToken);
                        putWithValidation(FIELD_ADDRESS, address);
                        break;
                    } catch (RuntimeException e) {  /* continue */ } catch (AddressFormatException e) {
                        e.printStackTrace();
                    }
                }else if (possibleType instanceof RippleFamily) {
                    try {
                        address = new RippleAddress(possibleType, addressToken);
                        putWithValidation(FIELD_ADDRESS, address);
                        break;
                    } catch (RuntimeException e) {  /* continue */ }
                } else {
     ...
     *
     * @return The {@link com.onets.core.coins.CoinType} of this URI
     */
