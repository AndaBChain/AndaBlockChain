...
import com.onets.core.util.AndaAddressUtils;
import com.onets.core.wallet.families.andachain.AndaAddress;
import com.onets.core.wallet.families.andachain.AndaBlockChainConnection;
import com.onets.core.wallet.families.andachain.AndaTransaction;
import com.onets.core.wallet.families.andachain.AndaTransactionEventListener;
import com.onets.core.wallet.families.andachain.AndaWalletTransaction;
import com.onets.core.wallet.families.bitcoin.OutPointOutput;
import com.onets.core.wallet.families.bitcoin.TrimmedOutPoint;
 ...
 * @author Yu K.Q.
 */
abstract public class TransactionWatcherWallet_Anda extends AbstractWallet<AndaTransaction, AndaAddress>
        implements TransactionBag, AndaTransactionEventListener {
    private static final Logger log = LoggerFactory.getLogger(TransactionWatcherWallet_Anda.class);
    ...
    final transient Map<Sha256Hash, Map.Entry<AndaTransaction, Set<Sha256Hash>>> outOfOrderTransactions;
    ...
    @VisibleForTesting final Map<Sha256Hash, AndaTransaction> pending;
    @VisibleForTesting final Map<Sha256Hash, AndaTransaction> confirmed;

    // All transactions together.
    final Map<Sha256Hash, AndaTransaction> rawTransactions;
    private AndaBlockChainConnection blockchainConnection;
    ...
    // Constructor
    public TransactionWatcherWallet_Anda(CoinType coinType, String id) {
        super(coinType, id);
     ...
     */
    public Iterable<AndaWalletTransaction> getWalletTransactions() {
        lock.lock();
        try {
            Set<AndaWalletTransaction> all = new HashSet<>();
     ...
     */
    private void simpleAddTransaction(WalletTransaction.Pool pool, AndaTransaction tx) {
        lock.lock();
        try {
    ...
    private static void addWalletTransactionsToSet(Set<AndaWalletTransaction> txs,
                                                   WalletTransaction.Pool poolType, Collection<AndaTransaction> pool) {
        for (AndaTransaction tx : pool) {
            txs.add(new AndaWalletTransaction(poolType, tx));
        }
    }
     ...
     */
    public void addWalletTransaction(AndaWalletTransaction wtx) {
        lock.lock();
        try {
    ...
    private boolean trimTransaction(Sha256Hash hash) {
        ...
        simpleAddTransaction(txPool,
                AndaTransaction.fromTrimmed(hash, tx, valueSent, valueReceived, fee));
        return true;
    }
     ...
     */
    private void addWalletTransaction(@Nullable WalletTransaction.Pool pool, AndaTransaction tx,
                                      boolean save) {
        lock.lock();
        try {
            if (pool == null) {
...
//        tx.getConfidence().addEventListener(txConfidenceListener, Threading.SAME_THREAD);
    ...
    private void guessSource(AndaTransaction tx) {
        checkState(lock.isHeldByCurrentThread(), "Lock is held by another thread");
    ...
    public Transaction getRawTransaction(Sha256Hash hash) {
        lock.lock();
        try {
            AndaTransaction tx = rawTransactions.get(hash);
            if (tx != null) {
     ...
     */
    public HashMap<Sha256Hash, AndaTransaction> getTransactions(Set<Sha256Hash> hashes) {
        lock.lock();
        try {
            HashMap<Sha256Hash, AndaTransaction> txs = new HashMap<>();
    ...
    public void onNewBlock(BlockHeader header) {
        ...
        try {
            ...
            for (AndaTransaction tx : rawTransactions.values()) {
    ...
    private void maybeUpdateBlockDepth(AndaTransaction tx, boolean updateUtxoSet) {
        checkState(lock.isHeldByCurrentThread(), "Lock is held by another thread");
        ...
        try {
            if (statusPendingUpdates.containsKey(status.getAddress())) {
                if (status.isUnspentTxQueued() && !status.isUnspentTxStateApplied()) {
                    ...
                    HashMap<Sha256Hash, AndaTransaction> txs = getTransactions(txHashes);
    ...
    private void applyUnspentState(AddressStatus status, HashMap<Sha256Hash, AndaTransaction> txs) {
        checkState(lock.isHeldByCurrentThread(), "Lock is held by another thread");
        ...
        for (Map.Entry<TrimmedOutPoint, OutPointOutput> utxo : unspentOutputs.entrySet()) {
            if (AndaAddressUtils.producesAddress(utxo.getValue().getScriptPubKey(), address)) {
        ...
        for (UnspentTx unspentTx : status.getUnspentTxs()) {
            ...
            AndaTransaction tx = checkNotNull(txs.get(outPoint.getHash()));
    ...
    private void applyHistoryState(AddressStatus status, HashMap<Sha256Hash, AndaTransaction> txs) {
        checkState(lock.isHeldByCurrentThread(), "Lock is held by another thread");
        ...
        for (HistoryTx historyTx : status.getHistoryTxs()) {
            AndaTransaction tx = checkNotNull(txs.get(historyTx.getTxHash()));
    ...
    private void checkTxConfirmation(HistoryTx historyTx, AndaTransaction tx) {
        checkState(lock.isHeldByCurrentThread(), "Lock is held by another thread");
    ...
    private void setAppearedAtChainHeight(AndaTransaction tx, int height, boolean updateUtxoSet) {
        checkState(lock.isHeldByCurrentThread(), "Lock is held by another thread");
    ...
    private void fetchTimestamp(AndaTransaction tx, Integer height) {
        checkState(lock.isHeldByCurrentThread(), "Lock is held by another thread");
     ...
     */
    private void maybeMovePool(AndaTransaction tx) {
        lock.lock();
        try {
            if (tx.getConfidenceType() == BUILDING) {
    ...
    void addNewTransactionIfNeeded(Transaction tx) {
        addNewTransactionIfNeeded(new AndaTransaction(tx));
    }

    @VisibleForTesting
    void addNewTransactionIfNeeded(AndaTransaction tx) {
        lock.lock();
        try {
            ...
            if (!rawTransactions.containsKey(hash) && !outOfOrderTransactions.containsKey(hash)) {
                ...
                for (Map.Entry<AndaTransaction, Set<Sha256Hash>> outOfOrderTx :
    ...
    @Override
    public void onTransactionUpdate(AndaTransaction tx) {
    ...
    @Override
    public void onTransactionBroadcast(AndaTransaction tx) {
        lock.lock();
        try {
    ...
    @Override
    public void onTransactionBroadcastError(AndaTransaction tx) {
    ...
    public void onConnection(BlockchainConnection blockchainConnection) {
        lock.lock();
        try {
            this.blockchainConnection = (AndaBlockChainConnection) blockchainConnection;
            clearTransientState();
    ...
    public void restoreWalletTransactions(ArrayList<WalletTransaction<AndaTransaction>> wtxs) {
        lock.lock();
        try {
            for (WalletTransaction<AndaTransaction> wtx : wtxs) {
                AndaTransaction tx = wtx.getTransaction();
    ...
    public Map<Sha256Hash, AndaTransaction> getAndaChainTransactionPool(WalletTransaction.Pool pool) {
        lock.lock();
        try {
            switch (pool) {
                case CONFIRMED:
    ...
    static Map<Sha256Hash, Transaction> toRawTransactions(Map<Sha256Hash, AndaTransaction> txs) {
        Map<Sha256Hash, Transaction> rawTxs = new HashMap<>(txs.size());
        for (Map.Entry<Sha256Hash, AndaTransaction> tx : txs.entrySet()) {
            ...
            for (final ListenerRegistration<WalletAccountEventListener> registration : listeners) {
                registration.executor.execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        registration.listener.onWalletChanged(TransactionWatcherWallet_Anda.this);
                    }
                });
            }
        }
    ...
    void queueOnNewBlock() {
        ...
        for (final ListenerRegistration<WalletAccountEventListener> registration : listeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
                    registration.listener.onNewBlock(TransactionWatcherWallet_Anda.this);
                    registration.listener.onWalletChanged(TransactionWatcherWallet_Anda.this);
                }
            });
        }
    ...
    void queueOnTransactionBroadcastSuccess(final AndaTransaction tx) {
        for (final ListenerRegistration<WalletAccountEventListener> registration : listeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
                    registration.listener.onTransactionBroadcastSuccess(TransactionWatcherWallet_Anda.this, tx);
                }
            });
        }
    }

    void queueOnTransactionBroadcastFailure(final AndaTransaction tx) {
        for (final ListenerRegistration<WalletAccountEventListener> registration : listeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
                    registration.listener.onTransactionBroadcastFailure(TransactionWatcherWallet_Anda.this, tx);
                }
            });
        }
    }
    ...
    String TAG = "------------TransactionWatcherWallet";
    @Override
    public void broadcastTx(AbstractTransaction tx) throws TransactionBroadcastException {
        Log.i(TAG, "broadcastTx: "+tx.getType().getName());
        if (tx instanceof AndaTransaction) {
            // TODO throw transaction broadcast exception
            broadcastTx((AndaTransaction) tx, this);
        } else {
    ...
    public boolean broadcastTxSync(AbstractTransaction tx) throws TransactionBroadcastException {
        if (tx instanceof AndaTransaction) {
            return broadcastBitTxSync((AndaTransaction) tx);
        } else {
            throw new TransactionBroadcastException("Unsupported transaction class: " +
                    tx.getClass().getName() + ", need: " + AndaTransaction.class.getName());
        }
    }

    private boolean broadcastBitTxSync(AndaTransaction tx) throws TransactionBroadcastException {
        if (isConnected()) {
            lock.lock();
            try {
    ...
    private void broadcastTx(AndaTransaction tx, TransactionEventListener<AndaTransaction> listener)
    ...
    Map<TrimmedOutPoint, OutPointOutput> getUnspentOutputs(boolean includeUnsafe) {
        ...
        try {
            ...
            for (AndaTransaction tx : pending.values()) {
    ...
    @Nullable
    public AndaTransaction getTransaction(Sha256Hash txId) {
        lock.lock();
        try {
    ...
    @Nullable
    @Override
    public AndaTransaction getTransaction(String transactionId) {
        lock.lock();
        try {
    ...
    @Override
    public Map<Sha256Hash, AndaTransaction> getTransactions() {
        lock.lock();
        try {
    ...
    @Override
    public Map<Sha256Hash, AndaTransaction> getPendingTransactions() {
        lock.lock();
        try {
