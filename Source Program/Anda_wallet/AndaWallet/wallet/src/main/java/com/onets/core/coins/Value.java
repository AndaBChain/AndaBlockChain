 ...
 * 货币价值，不可变。
 */
public class Value implements MyMonetary, Comparable<Value>, Serializable, Monetary {
     ...
     */
    public final BigInteger value_big;
    public final long value;

    Value(final ValueType type, final long units) {
        this.type = checkNotNull(type);
        this.value = units;
        this.value_big = new BigInteger(String.valueOf(units));
    }

    protected Value(final ValueType type, final BigInteger units) {
        this.type = checkNotNull(type);
        this.value_big = units;
        this.value = 0l;
    }
    ...
    public static Value valueOfEth(final ValueType type, String unitsStr) {
        Log.e(TAG, "type.getMinNonDust:" + type.getMinNonDust() + "   type.getUnitExponent:" + type.getUnitExponent() + "   type.getName:" + type.getName() + "   Value: " + new BigInteger(unitsStr));
        return valueOfEth(type, new BigInteger(unitsStr));
    }

    public static Value valueOfEth(final ValueType type, BigInteger units) {
        return new Value(type, units);
    }
    ...
    public Coin toBitCoin() {
        return Coin.valueOf(value);
    }

    public Coin toAndaCoin(){
        return Coin.valueOf(value);
    }

    public Coin toEthCoin() {
        return Coin.valueOf(value);
    } //可能会重写
    ...
    public static Value parse(final ValueType type, final String str) {
        if(type.getName().equals("Ethereum")){
            return parseEth(type, new BigDecimal(str));
        }else if (type.getName().equals("AndaBlockChain")){
    ...
    public static Value parseEth(final ValueType type, final BigDecimal decimal) {
        return Value.valueOfEth(type, decimal.movePointRight(type.getUnitExponent())
                .toBigIntegerExact());
    }


    //------------------------------ 以太坊  -----------------------------------------------
    public Value addBig(final Value value) {
        checkArgument(type.equals(value.type), "Cannot add a different type");
        return new Value(this.type, this.value_big.add(value.value_big));
    }

    //  todo :  待修改
    public Value addBig(final Coin value) {
        return new Value(this.type, this.value_big.add(new BigInteger(String.valueOf(value.value))));
    }

    public Value addBig(final BigInteger value) {
        return new Value(this.type, this.value_big.add(value));
    }

    public Value subtractBig(final Value value) {
        checkArgument(type.equals(value.type), "Cannot subtract a different type");
        return new Value(this.type, this.value_big.subtract(value.value_big));
    }

    public Value subtractBig(final Coin value) {
        return new Value(this.type, LongMath.checkedSubtract(this.value, value.value));
    }

    public Value subtractBig(String str) {
        return subtractBig(type.value(str));
    }

    public Value subtractBig(BigInteger value) {
        return new Value(this.type, this.value_big.subtract(value));
    }

    public Value multiplyBig(final BigInteger factor) {
        return new Value(this.type, this.value_big.multiply(factor));
    }

    public Value divideBig(final BigInteger divisor) {
        return new Value(this.type, this.value_big.divide(divisor));
    }

    public Value[] divideAndRemainderBig(final BigInteger divisor) {
        return new Value[]{new Value(this.type, this.value_big.divide(divisor)),
                new Value(this.type, this.value_big.mod(divisor))};
    }

    public BigInteger divideBig(final Value divisor) {
        checkArgument(type.equals(divisor.type), "Cannot divide with a different type");
        return this.value_big.divide(divisor.value_big);
    }
    ...
    public Value multiplyBig(final long factor) {
        return new Value(this.type, this.value_big.multiply(new BigInteger(String.valueOf(factor))));
    }
    ...
    @Override
    public int signum() {
        Log.i(TAG, "signum: " + type.getName());
        if (type.getName().equals("Ethereum")) {//以太坊
            Log.i(TAG, "signum: "+value_big);
            if (this.value_big.compareTo(BigInteger.ZERO) == 0) {
                Log.i(TAG, "signum: value_big = 0");
                return 0;
            }
            Log.i(TAG, "signum: value_big != 0");
            return this.value_big.compareTo(BigInteger.ZERO) == -1 ? -1 : 1;
        }else if (type.getName().equals("AndaBlockChain")){//安达币
            if (this.value == 0){
                return 0;
            }
    ...
    public String toPlainString() {
        if(type.getName().equals("Ethereum")){
            return type.getPlainFormat().formatEth(this,18).toString();
        }else if(type.getName().equals("AndaBlockChain")){
    ...
    public int compareTo(@Nonnull final Value other) {
        ...
        if(other.type.getName().equals("Ethereum")) {
            if (this.value_big.compareTo(other.value_big) == 0)
                return 0;
            return this.value_big.compareTo(other.value_big ) == 1 ? 1 : -1;
        }else if (type.getName().equals("AndaBlockChain")){
