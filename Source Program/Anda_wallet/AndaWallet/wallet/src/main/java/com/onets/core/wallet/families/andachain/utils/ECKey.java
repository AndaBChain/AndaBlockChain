...
public class ECKey implements Serializable {
    ...
    private static final long serialVersionUID = -728224901792295832L;//序列化版本号

    private final BigInteger privKey;//私钥
    protected final ECPoint pub;//公钥（椭圆曲线）


    private static final SM2 sm2;
    // Transient because it's calculated on demand.
    transient private byte[] pubKeyHash;//公钥hash
    transient private byte[] nodeId;

    public static final BigInteger HALF_CURVE_ORDER;//一般曲线

    public  static final ECDomainParameters CURVE;//曲线

    static {
        sm2 = SM2.Instance();
        CURVE = sm2.ecc_bc_spec;
    ...
    public ECKey(SM2 sm2) {

        AsymmetricCipherKeyPair key = sm2.ecc_key_pair_generator.generateKeyPair();

        org.spongycastle.crypto.params.ECPrivateKeyParameters ecpriv = (org.spongycastle.crypto.params.ECPrivateKeyParameters) key.getPrivate();
        org.spongycastle.crypto.params.ECPublicKeyParameters pubKey = (org.spongycastle.crypto.params.ECPublicKeyParameters) key.getPublic();

        this.privKey = ecpriv.getD();
        pub = pubKey.getQ();

//        System.out.println("公钥 0: " + Util.byteToHex(pub.getEncoded()));
//        System.out.println("私钥 0: " + Util.byteToHex(privKey.toByteArray()));
    }

    public ECKey(BigInteger privKey, ECPoint multiply) {
        this.privKey = privKey;
        this.pub = multiply;
//        System.out.println("公钥 1: " + Util.byteToHex(pub.getEncoded()));
//        System.out.println("私钥 1: " + Util.byteToHex(privKey.toByteArray()));
    }
    ...
    public ECKey decompress() {
        if (!pub.isCompressed())
            ...
            return new ECKey(this.privKey, decompressPoint(pub));
    }
    ...
    public ECKey compress() {
        if (pub.isCompressed())
            ...
            return new ECKey(this.privKey, compressPoint(pub));
    }
    ...
    public static ECKey fromNodeId(byte[] nodeId) {
        check(nodeId.length == 64, "Expected a 64 byte node id,but this length is "+nodeId.length);
    ...
    public String toStringWithPrivate() {
        ...
        b.append(toString());
        if (privKey != null) {
            b.append(" priv:").append(Hex.toHexString(privKey.toByteArray()));
        }
        return b.toString();
    }
     ...
     * how SM2 signatures are represented when embedded in other data structures in the AnDaChain protocol. The raw
    ...
    public static class SM2Signature {
         ...
         * the same message. However, we dislike the ability to modify the bits of a AnDaChain transaction after it's
        ...
        public SM2Signature toCanonicalised() {
            if (s.compareTo(HALF_CURVE_ORDER) > 0) {
                ...
                return new SM2Signature(r, CURVE.getN().subtract(s));
            } else {
                return this;
            }
        }
     ...
     * and put them in SM2Signature
     *
     * @param input to sign
     * @return SM2Signature signature that contains the R and S components
     */
    public SM2Signature doSign(byte[] input) throws IOException {
        if (input.length != 32) {
            throw new IllegalArgumentException("Expected 32 byte input to SM2 signature, not " + input.length);
        }
        ...
        if (privKey == null)
            ...
            ECPrivateKeyParameters privKeyParams = new ECPrivateKeyParameters(privKey, CURVE);
            signer.init(true, privKeyParams);
            BigInteger[] components = signer.generateSignature(input);
            return new SM2Signature(components[0], components[1]).toCanonicalised();
//        }
//        final byte[] derSignature =SM2Utils.sign(getNodeId(),privKey.toByteArray(),input);
//        return SM2Signature.decodeFromDER(derSignature).toCanonicalised();
    }
     ...
     */
    public SM2Signature sign(byte[] messageHash) throws IOException {
        SM2Signature sig = doSign(messageHash);
     ...
     * @param signatureBase64 The AnDaChain-format message signature in base64
     *
     * @return -
     ...
     * @return ECKey
     */
    public static ECKey signatureToKey(byte[] messageHash, SM2Signature sig) throws SignatureException {
    ...
    public BigInteger keyAgreement(ECPoint otherParty) {
        if (privKey == null) {
            ...
            agreement.init(new ECPrivateKeyParameters(privKey, CURVE));
...
//        }
        ...
        KeyParameter key = new KeyParameter(BigIntegers.asUnsignedByteArray(privKey));
     ...
     */
    @Nullable
    public static ECKey recoverFromSignature(int recId, SM2Signature sig, byte[] messageHash) {
        final byte[] pubBytes = recoverPubBytesFromSignature(recId, sig, messageHash);
        if (pubBytes == null) {
            return null;
        } else {
    ...
    public byte[] getPrivKeyBytes() {
        if (privKey == null) {
            ...
            return bigIntegerToBytes(privKey, 32);
        }
    }
