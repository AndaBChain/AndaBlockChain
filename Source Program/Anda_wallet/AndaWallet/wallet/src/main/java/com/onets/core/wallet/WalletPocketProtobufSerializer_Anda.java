...
import com.onets.core.wallet.families.bitcoin.EmptyTransactionOutput;
import com.onets.core.wallet.families.bitcoin.OutPointOutput;
import com.onets.core.wallet.families.bitcoin.TrimmedOutput;
 ...
 * @author Yu K.Q.
 */
public class WalletPocketProtobufSerializer_Anda {
    private static final Logger log = LoggerFactory.getLogger(WalletPocketProtobufSerializer_Anda.class);

    // Used for de-serialization
    protected Map<ByteString, AndaTransaction> txMap = new HashMap<>();

    public static Protos.WalletPocket toProtobuf(WalletPocketHD_Anda pocket) {
    ...
    private static Protos.Transaction makeTxProto(WalletTransaction wtx) {
        if (wtx instanceof AndaWalletTransaction) {
            return makeBitTxProto((AndaWalletTransaction) wtx);
        } else {
    ...
    private static Protos.Transaction makeBitTxProto(AndaWalletTransaction wtx) {
        AndaTransaction bitTx = wtx.getTransaction();
...
//            }
     ...
     */
    public WalletPocketHD_Anda readWallet(Protos.WalletPocket walletProto, @Nullable KeyCrypter keyCrypter) throws UnreadableWalletException {
        CoinType coinType;
        try {
        ...
        WalletPocketHD_Anda pocket;
        if (walletProto.hasId()) {
            pocket = new WalletPocketHD_Anda(walletProto.getId(), chain, coinType);
        } else {
            pocket = new WalletPocketHD_Anda(chain, coinType);
        }
        ...
        try {
            ...
            ArrayList<WalletTransaction<AndaTransaction>> wtxs = new ArrayList<>(walletProto.getTransactionList().size());
            for (Protos.Transaction txProto : walletProto.getTransactionList()) {
...
//        tx.setPurpose(Transaction.Purpose.USER_PAYMENT);
        ...
        if (isTrimmed) {
            ...
            bitTx = AndaTransaction.fromTrimmed(byteStringToHash(txProto.getHash()), tx, valueSent,
                    valueReceived, fee);
        } else {
            bitTx = new AndaTransaction(tx);
        }
    ...
    private AndaWalletTransaction connectTransactionOutputs(Protos.Transaction txProto) throws UnreadableWalletException {
        AndaTransaction tx = txMap.get(txProto.getHash());
        ...
        for (int i = 0 ; i < tx.getOutputs(false).size() ; i++) {
            ...
            if (tx.isTrimmed()) {
                ...
                if (transactionOutput.hasSpentByTransactionHash()) {
                    ...
                    AndaTransaction spendingTx = txMap.get(spentByTransactionHash);
        ...
        return new AndaWalletTransaction(pool, tx);
    }
