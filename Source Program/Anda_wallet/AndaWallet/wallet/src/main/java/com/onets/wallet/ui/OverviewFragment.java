...
import com.onets.wallet.ui.adaptors.AccountListAdapter;
...
public class OverviewFragment extends Fragment {
    ...
    private static class MyHandler extends WeakHandler<OverviewFragment> {
        ...
        protected void weakHandleMessage(OverviewFragment ref, Message msg) {
            switch (msg.what) {
                case WALLET_CHANGED://更改钱包0
                    ref.updateWallet();
                    break;
                case SET_EXCHANGE_RATES://设置汇率2
                    ref.setExchangeRates((Map<String, ExchangeRate>) msg.obj);
                    break;
                case UPDATE_VIEW://更新视图1
                    ref.updateView();
                    break;
            }
        }
    ...
    private Value currentBalance;//最近余额

    private boolean isFullAmount = false;
    private WalletApplication application;//钱包应用
    private Configuration config;//配置

    private AccountListAdapter adapter;//账户列表适配器
    Map<String, ExchangeRate> exchangeRates;//汇率
    ...
    private void calculateNewBalance() {
        ...
        Log.e("---------------OverView", "all wallet count :" + wallet.getAllAccounts().size());
        for (WalletAccount w : wallet.getAllAccounts()) {
            ...
            if (w.getCoinType().getSymbol().equals("ETH")) {
                if (currentBalance != null) {
                    Log.e("------------OverView", "calculateNewBalance: currentBalance:" + currentBalance
                            + "\ngetBalance : " + w.getBalance()
                            + "\nrate.rate.Convert(w.getBalance()) : " + rate.rate.convert(w.getBalance())
                    );
                }
            }
            ...
            if (w.getCoinType().getName().equals("AndaBlockChain")) {
