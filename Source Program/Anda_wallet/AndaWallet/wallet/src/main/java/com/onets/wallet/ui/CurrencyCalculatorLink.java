...
import com.onets.wallet.ui.widget.AmountEditView.Listener;
...
public final class CurrencyCalculatorLink {
    ...
    private final Listener coinAmountViewListener = new Listener() {
        @Override
        public void changed() {
    ...
    private final Listener localAmountViewListener = new Listener() {
        @Override
        public void changed() {
    ...
    public Coin getPrimaryAmountCoin() {
        ...
        return value != null ? value.toBitCoin() : null;
    }
    ...
    private Value convertSafe(Value amount) {
        try {
            Log.i("^^^^^^^^^^^^^^^^^^^^^^", "convertSafe: "+amount.getBigValue());
